...
## 草图识别
在草图识别部分，本项目利用[TorchSketch](https://github.com/PengBoXiangShang/torchsketch)中的CNN模型和RNN模型
实现了草图识别模型的训练。其中，CNN模型基于数据集中`.png`图像文件进行训练，而RNN模型基于数据集中`.npy`序列文件进行训练。\
